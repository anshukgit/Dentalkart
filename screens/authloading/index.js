...
export default class AuthLoading extends Component {
  ...
  handleInitialScreen = async (res) => {
    console.log("NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN", res);

    if (res?.data?.link) {
      const fullUrl = res.data.link;
      console.log("Full URL:", fullUrl);

      // Manually extract the pathname from the full URL
      const urlKey = fullUrl
        .replace("https://www.dentalkart.com", "")
        .split("?")[0];
      console.log("Extracted Pathname:", urlKey);

      // Now you can navigate using the extracted urlKey
      return setTimeout(() => {
