...
class BrandList extends Component {
  ...
  getSearchValues = (key) => {
    ...
    const filteredSearch = value.filter((item) =>
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View>
            <View style={styles.brandListTopWrapper}>
              <Text allowFontScaling={false} style={styles.brandTopText}>
                All Brands
              </Text>
              <TextInput
  ...
  render() {
    return (
      <View style={styles.BrandWrapper}>
        <Query query={GET_BRANDS} fetchPolicy="network-only" client={newclient}>
          {({data, loading, error}) => {
            if (data && data.getBrand) {
              ...
              return (
                <Text allowFontScaling={false}>There has been some issue. Please reload & check again.</Text>
              );
            } else if (loading) {
              return <Text style={Styles.loading} allowFontScaling={false}> Loading...</Text>;
            } else return null;
          }}
        </Query>
      </View>
