  ...
  return (
    <View>
      <View style={AttachmentStyle.attachButtons}>
        <Text allowFontScaling={false} style={AttachmentStyle.attachText}>
          Add attachments -{' '}
        </Text>
        <TouchableOpacity onPress={() => capturePic(instance)}>
          <View style={AttachmentStyle.attachIconWrapper}>
            <Icon
              name="ios-camera"
              style={AttachmentStyle.attachButtonCameraIcon}
            />
          </View>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => imagePicker(instance)}>
          <View style={AttachmentStyle.attachIconWrapper}>
            <Icon
              ...
              style={AttachmentStyle.attachButtonFolderIcon}
            />
          </View>
        </TouchableOpacity>
      </View>
      <Text allowFontScaling={false} style={AttachmentStyle.imageInfo}>
        Only .jpeg, .jpg and .png formats are supported. The size must be less
        than 10MB
      </Text>
      <View>
        <FlatList
          ...
          renderItem={({item, index}) => (
            <AttachedImage item={item} instance={instance} index={index} />
          )}
...
const AttachedImage = ({instance, item, index}) => {
  return (
    <View style={AttachmentStyle.attachedImageWrapper}>
      <TouchableOpacity
        onPress={() => removeAttachment(instance, index)}
        style={AttachmentStyle.attachedRemoveIconWrapper}>
        <Icon
          name="ios-close-circle-outline"
          style={AttachmentStyle.attachedRemoveIcon}
        />
      </TouchableOpacity>
      <Image
        resizeMethod={'resize'}
        source={{uri: item.uri}}
        style={AttachmentStyle.attachedImage}
      />
    </View>
  );
};
